...
Holiday Status = 
    IF('fmcg_sales_3years_1M_rows'[is_holiday]=FALSE()," No Holiday","Holiday")

       
2. promo group = 
	IF('fmcg_sales_3years_1M_rows'[discount_pct]=0,"No Discount",
 	IF('fmcg_sales_3years_1M_rows'[discount_pct]<0.10,"Below 10%",
  	IF('fmcg_sales_3years_1M_rows'[discount_pct]<0.20,"Below 20%",
   	IF('fmcg_sales_3years_1M_rows'[discount_pct],"Below 30%"))))
        
3. promotion = 
	IF('fmcg_sales_3years_1M_rows'[promo_flag]=FALSE(),"No Discount","Discount")

4. Rain grouping = 
	IF( 'fmcg_sales_3years_1M_rows'[rain_mm]=0,"No rain", 
	IF('fmcg_sales_3years_1M_rows'[rain_mm]<=0.99,"Very little"	,
	IF('fmcg_sales_3years_1M_rows'[rain_mm]<=2.99,"little rain",
	IF('fmcg_sales_3years_1M_rows'[rain_mm]<=4.99,"Much rain",
	IF('fmcg_sales_3years_1M_rows'[rain_mm]<=5.99,"Heavy Rain",
	IF('fmcg_sales_3years_1M_rows'[rain_mm]<=8.99,"Slightly heavy",
	IF('fmcg_sales_3years_1M_rows'[rain_mm]>8.99,"Very heavy")
))))))

5. weekend vs weekday =
	IF('fmcg_sales_3years_1M_rows'[is_weekend]=TRUE(),"Weekends","Weekday")
...
3. Promo Lift % = 
DIVIDE(
    CALCULATE(AVERAGE('fmcg_sales_3years_1M_rows'[units_sold]),'fmcg_sales_3years_1M_rows'[promo_flag]=TRUE())-
    CALCULATE(AVERAGE('fmcg_sales_3years_1M_rows'[units_sold]), 'fmcg_sales_3years_1M_rows'[promo_flag]=FALSE()),
    CALCULATE(AVERAGE('fmcg_sales_3years_1M_rows'[units_sold]), 'fmcg_sales_3years_1M_rows'[promo_flag]=FALSE()))

4. Stock Out Count = 
CALCULATE(
    COUNTROWS('fmcg_sales_3years_1M_rows'),
    'fmcg_sales_3years_1M_rows'[stock_out_flag]=TRUE())

5. Stockout Rate(%) = DIVIDE(
    COUNTROWS(FILTER('fmcg_sales_3years_1M_rows','fmcg_sales_3years_1M_rows'[stock_out_flag]=TRUE())),COUNTROWS('fmcg_sales_3years_1M_rows'))*100

6. Total Profit = 
 SUM('fmcg_sales_3years_1M_rows'[net_sales]) - 
 SUMX('fmcg_sales_3years_1M_rows','fmcg_sales_3years_1M_rows'[purchase_cost]
 *
 'fmcg_sales_3years_1M_rows'[units_sold])

7 . Total Sales = 
  SUM(fmcg_sales_3years_1M_rows[net_sales]  )

8. Total Unit Sold = 
   SUM('fmcg_sales_3years_1M_rows'[units_sold])
